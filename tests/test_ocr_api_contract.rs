...
async fn test_ocr_request_contract_structure() {
    ...
    assert!(
        ...
        "Request must have 'model' field"
    );
    assert!(
        json.get("document").is_some(),
        "Request must have 'document' field"
    );
    ...
    assert_eq!(
        ...
        "mistral-ocr-latest",
        "Model must be 'mistral-ocr-latest'"
    );

    // Validate document structure
    let document = json.get("document").unwrap();
    assert!(
        document.get("type").is_some(),
        "Document must have 'type' field"
    );
    assert!(
        document.get("file_id").is_some(),
        "Document must have 'file_id' field"
    );

    assert_eq!(
        document.get("type").unwrap().as_str().unwrap(),
        "file",
        "Document type must be 'file'"
    );

    assert_eq!(
        document.get("file_id").unwrap().as_str().unwrap(),
        "file-abc123",
        "File ID must match expected value"
    );
}
...
async fn test_ocr_request_contract_required_fields() {
    ...
    assert!(
        json.get("model").is_some(),
        "Required field 'model' missing from request"
    );

    assert!(
        json.get("document").is_some(),
        "Required field 'document' missing from request"
    );

    let model = json.get("model").unwrap().as_str().unwrap();
    assert!(
        !model.is_empty(),
        "Model must not be empty, got '{}'",
        model
    );
...
async fn test_ocr_request_contract_validation() {
    ...
    assert!(
        ...
        "Valid file_id should pass validation"
    );
    ...
    assert!(
        ...
        "Empty file_id should fail validation"
    );
}
...
async fn test_ocr_response_contract_structure() {
    ...
    assert_eq!(
        response.model, "mistral-ocr-2505-completion",
        "Model field must match"
    );
    assert_eq!(response.pages[0].index, 0, "Page index must be 0");
    assert_eq!(
        response.pages[0].markdown, "This is the extracted text from the document.",
        "Markdown content must match"
    );
}
...
async fn test_ocr_response_contract_required_fields() {
    ...
    assert_eq!(response.pages[0].index, 0, "Page index must be 0");
    assert!(
        !response.pages[0].markdown.is_empty(),
        "Page markdown content is required"
    );
}
...
async fn test_ocr_response_contract_pages_structure() {
    ...
    assert_eq!(
        page.markdown, "This is the OCR extracted text.",
        "Page markdown must match"
    );
...
async fn test_ocr_response_contract_usage_info() {
    ...
    assert_eq!(
        ...
        "Pages processed must match"
    );
    assert_eq!(
        response.usage_info.doc_size_bytes, 469,
        "Document size must match"
    );
}
...
async fn test_ocr_response_contract_extracted_text_access() {
    ...
    assert_eq!(
        extracted_text, "This is the extracted OCR text from the document.",
        "Extracted text must match page markdown"
    );

    // Test helper method
    let extracted_via_helper = response.get_extracted_text();
    assert_eq!(
        extracted_via_helper, "This is the extracted OCR text from the document.",
        "Helper method should return same text as direct access"
    );
}
...
async fn test_ocr_response_contract_validation() {
    ...
    assert!(
        ...
        "Invalid response (empty pages) should fail validation"
    );
}
...
async fn test_ocr_response_contract_multiple_pages() {
    ...
    assert_eq!(
        response.pages[0].markdown, "First page content",
        "First page content must match"
    );
    assert_eq!(
        response.pages[1].markdown, "Second page content",
        "Second page content must match"
    );
    ...
    assert_eq!(
        ...
        "Helper method should concatenate multiple pages with newlines"
    );
}
