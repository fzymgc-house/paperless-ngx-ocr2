...
async fn test_cli_json_output_flag() {
    ...
    if output.status.success() {
        ...
        if json.get("success").unwrap().as_bool().unwrap() {
            assert!(json.get("data").is_some(), "Successful JSON output must have 'data' field");
        } else {
            assert!(json.get("error").is_some(), "Failed JSON output must have 'error' field");
        }
    }
