...
async fn test_comprehensive_utility_demonstration() {
    ...
    let pdf_fixture = fixtures::sample_pdf();
    let png_fixture = fixtures::sample_png();
    ...
    measure_performance("demo_operation", Duration::from_millis(100), || {
        // Simulate some work
...
fn test_utility_error_handling() {
    ...
    let test = PerformanceTest::new("timeout_test", Duration::from_millis(1));
    std::thread::sleep(Duration::from_millis(10));
    // This should panic when assert_within_time is called
    // We don't call it here to avoid panicking in the test
}
...
fn test_utility_combinations() {
    ...
    let test_file = create_test_pdf("Combination test");
