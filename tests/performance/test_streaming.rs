...
async fn test_streaming_threshold_detection() {
    ...
    let form_result =
        FileUploadRequest::to_streaming_multipart_form(temp_path.to_str().unwrap(), "ocr").await;
    let duration = start.elapsed();

    assert!(
        form_result.is_ok(),
        "Should create streaming form for large file"
    );
    assert!(
        duration.as_millis() < 1000,
        "Streaming form creation should be fast: {:?}",
        duration
    );
...
async fn test_streaming_memory_efficiency() {
    ...
    assert!(
        ...
        "Streaming should be fast regardless of file size: {:?}",
        duration
    );
...
async fn test_regular_vs_streaming_threshold() {
    ...
    assert!(
        ...
        "Regular form creation should be very fast: {:?}",
        duration
    );
...
async fn test_streaming_form_validation() {
    ...
    temp_file.write_all(b"%PDF-1.4\n").unwrap();
    temp_file
        .write_all(b"Test content for streaming validation.\n")
        .unwrap();
    ...
    let form = FileUploadRequest::to_streaming_multipart_form(temp_path.to_str().unwrap(), "ocr")
        .await
        .expect("Should create streaming form");
