...
fn test_memory_efficient_file_reading() {
    ...
    assert!(
        validation_duration.as_millis() < 50,
    ...
    assert!(
        ...
        "File read should be reasonable: {:?}",
        read_duration
    );
...
fn test_file_upload_request_memory_usage() {
    ...
    assert!(
        ...
        "Request creation should be fast: {:?}",
        creation_duration
    );
    ...
    assert!(
        validation_duration.as_millis() < 10,
    ...
    assert!(
        form_duration.as_millis() < 200,
        "Multipart form creation should be reasonable: {:?}",
        form_duration
    );
...
fn test_memory_usage_with_multiple_files() {
    ...
    for size in file_sizes {
        ...
        assert!(
            ...
            "File processing took too long for {} bytes: {:?}",
            size,
            duration
        );
...
fn test_streaming_validation_efficiency() {
    ...
    assert!(
        validation_duration.as_millis() < 20,
        "Validation should be constant time: {:?}",
        validation_duration
    );
    assert!(file_upload.file_size > 5_000_000); // Should be > 5MB
